...
skins = ["default", "aqua", "black", "bloody", "cobalt", "gold", "insta",
         ...
         "purple", "rgb", "silver", "blue", "sunny", "toxic"]
names = ["Flatter", "React", "MAUI", "Deno",
         "Swift", "Node.js", "SQL", "C++",
...
class Dino:
    ...
    def __init__(self, x, y, skin="default", name=None):
        self.skin = skin
        self.load_sprites()
    ...
    def load_sprites(self):
        self.sprites["jump"].append(pygame.image.load(f"sprites/dino/{self.skin}_jump.png"))
        self.sprites["run"].append(pygame.image.load(f"sprites/dino/{self.skin}_run1.png"))
        self.sprites["run"].append(pygame.image.load(f"sprites/dino/{self.skin}_run2.png"))
    ...
    def run(self):
        self.sprites["run"][0] = pygame.image.load(f"sprites/dino/{self.skin}_run1.png")
        self.sprites["run"][1] = pygame.image.load(f"sprites/dino/{self.skin}_run2.png")
    ...
    def jump(self):
        if self.state == DinoState.JUMP:
            ...
            self.image = pygame.image.load(f"sprites/dino/{self.skin}_jump.png")
...
def run_game(genomes, config):
    global game_speed, score, enemies, dinos, generation, score_speedup
    ...
    for i, g in genomes:
        ...
        dinos.append(Dino(30, height - 170, skin, name))
    ...
    # dinos = [Dino(30, height-170, "default", "Lucifer")]
    ...
    while True:
        ...
        for i, dino in enumerate(dinos):
            output = nets[i].activate((dino.hitbox.y,
                                       calc_dist((dino.hitbox.x, dino.hitbox.y), enemies[0].hitbox.midtop),
                                       enemies[0].hitbox.width,
                                       game_speed))

            if output[0] > 0.5 and dino.state is not DinoState.JUMP:
                dino.jump()
        ...
        score_label = score_font.render("Points: " + str(math.floor(score)), True, (50, 50, 50))
        ...
        # display dinos names
        for i, dino in enumerate(dinos):
            dname_label = dname_font.render(dino.name, True, (170, 238, 187))
